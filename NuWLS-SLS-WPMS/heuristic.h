...
int ISDist::pick_var()
{
    ...
    if (goodvar_stack_fill_pointer > 0)
    {
        ...
        else
        {
            ...
            //cout << "c good score: " << score[best_var] << endl;
            return best_var; //best_array[rand() % best_array_count];
        }
    }
...
void ISDist::local_search_with_decimation(char *inputfile)
{
    ...
    else if (total_soft_length / num_sclauses > 100)
    {
    ...
    for (tries = 1; tries < max_tries; ++tries)
    {
        ...
        //else // only one
        //    init(deci.fix);
        cout << "c tries: " << tries << " " << total_step << " " << h_inc << " " << s_inc << endl;
...
void ISDist::soft_increase_weights()
{
    int i, c, v;
    {
        ...
        {
            c = softunsat_stack[i];
            //if (clause_weight[c] >= org_clause_weight[c] + softclause_weight_threshold)
